...
<section class="content">
  <div class="box box-default">
    <div class="box-header with-border">
      <h3 class="box-title"><strong>INFORMACIÓN GENERAL (TIPO DE SOLICITUD)</strong></h3>
    </div>
    <div class="box-body">
    <!--INFORMACION GENERAL-->
      <div class="row">
        <div class="col-md-6 col-sm-6">
          <div class="form-group">
            ...
            <select class="form-control select2" id="select-tipo-documento" style="width: 100%;">
                    <?php
        ...
        <div id="pantalla-principal" style="display:none">
            ...
            <div class='row'>
                <div class="col-md-12 OPCIONES ECE ECL INT PRO SGE QUE QUI REC REI REP" style="display:none">
                    <div class="form-group">
                        <button type='button' class='btn btn-primary' id='' onclick="generarSolicitud()" ><span class='fa fa-save'> Generar</span></button>                
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">          
            <form enctype="multipart/form-data" id="form1">
                ...
                <input type="hidden" id="codigoSolicitudDocumento2" name="codigoSolicitudDocumento2" value="SO">
...
$("#select-subtipo-documento").change(function(){
  ...
  $.ajax({
    ...
    if (codigoDocumento == "RE" ){

        console.log("cargando tipo reclamo");
        $.ajax({
            url: '/nqr/web/index.php?r=solicitud/obetenertipos_qu_re', 
            type : 'POST',      
            datatype : 'json',
            data : {'codigoSubtipo':codigoSubtipo}, 
                beforeSend: function() {   
                    $("#select-tipoReclamo").append('<option value="">cargando...</option>');
                },success: function(data) {
                    var obj = jQuery.parseJSON(data);
                    $("#select-tipoReclamo").empty();
                    $.each(obj, function(i, item) {                 
                        $("#select-tipoReclamo").append('<option value="'+item.tipoReclamo+'">'+item.tipoReclamo+'</option>');
                    });
...
function obtenerArchivos(){
    $.ajax({
        ...
        type : 'POST',      
        datatype : 'json',      
        data : {'opcion':8,'codigoSolicitudDocumento':'SO'}, 
            success: function(data) {
                var obj = jQuery.parseJSON(data);
    ...
    data : {'usuarioLogin':'JANCHUNDIA','subTipoDocumento':codigoSubtipo}, 
        beforeSend: function() {   
            ...
            $.each(obj, function(i, item) {                 
                ...
                $(".OPCIONES-DOC").show();
                //VALIDACION DE LA PANTALLA QUE SE DEBE MOSTRAR
                //VALIDACION DE CAMPOS QUE SE DEBEN BLOQUEAR
                if (codigoSubtipo != 'SN' )
                {    
                    //DEPENDIENDO DE EL SUBDOCUMENTO SE HABILITAN LAS OPCIONES
                    if (codigoSubtipo =="ECE"){
                        ...
                        $(".INT").show();  //USUARIO Y AREA EMISOR SON TOMADOS POR EL USUARIO QUE SE LOGEA
                        $(".USUARIO-EMISOR").val(item.usuarioEmisor);
                        $(".AREA-EMISOR").val(item.areaEmisor);//disabled
                        $(".USUARIO-RECEPTOR").prop('readonly', false);//DEBE OBTENER EL USUARIO DE ACUERDO AL MANTEMIENTO DE AREAS
                        $(".AREA-RECEPTOR").prop('readonly', false);//DEBE SELECCIONAR UN AREA DEL MANTENIMIENTO
                    }else if(codigoSubtipo =="PRO"){
                        $("#pantalla-principal").show();
...
function generarSolicitud()
{
    ...
    var valores = {'codigoDocumento':codigoDocumento,'codigoSubtipo':codigoSubtipo,'codauditoria':codauditoria
        ...
        if(areaEmisor.trim().length > 3){
            if(usuarioReceptor.trim().length > 3){
                if(areaReceptor.trim().length > 3){
                    ...
                    if (hallazgo.length  > 50){
                      //VALIDACION PARA EL TRATAMIENTO INMEDIATO
                      if(tratamientoInmediato.length >20){
                        swal({
                          title: "Generación de solicitud",
                          text: "Desea continuar con la generación?",
                          type: "info",
                          ...
                          showLoaderOnConfirm: true
                            ...
                            $.ajax({
                              url: '/nqr/web/index.php?r=solicitud/generarsolicitud', 
                              type : 'POST',      
                              datatype : 'json',
                              data : valores, 
                                success: function(data) {            
