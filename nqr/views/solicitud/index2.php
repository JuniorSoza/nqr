...
?>
    ...
    drilldownData = [];
    <?php
    foreach($dataDocumentos as $row2)
    {
        if ($row['descripcionDocumento'] == $row2['descripcionDocumento'])
        {        
        ?>
        var estadoSolicitudDesc = <?php echo "'".$row2['estadoSolicitudDesc']."'";?>;
...
Highcharts.chart('container', {
    ...
    title: {
        text: 'Cantidad de DOCUMENTOS generados por: <?php $session = Yii::$app->session; echo $session['usuarioSession']; ?>'
    },
    subtitle: {
        text: 'Documentos creados hasta la actualidad'
    },
    accessibility: {
        announceNewData: {
            enabled: true
